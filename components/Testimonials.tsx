      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-24 gap-8">
          ...
          <motion.div
            ...
            className="bg-card-bg px-8 py-4 rounded-full border border-muted/20"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          {TESTIMONIALS.map((testimonial, idx) => (
            <motion.div
              ...
              key={testimonial.id}
              className={`bg-card-bg/60 backdrop-blur-2xl p-12 rounded-[4rem] border border-muted/10 relative group transition-all duration-700 hover:border-thr/50 shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:shadow-[0_0_80px_rgba(139,92,246,0.2)] hover:-translate-y-4 overflow-hidden ${idx === 1 ? 'lg:-translate-y-12 lg:hover:-translate-y-16' : ''}`}
            >
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-[inherit]"
                style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.12) 0%, rgba(34,211,238,0.05) 100%)' }} />
